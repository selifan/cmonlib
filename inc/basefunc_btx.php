<?php
/**
* @name basefunc_btx.php - common functions for all php pages on the site,
* 1C-Bitrix Adopted version!
...
if(!defined('JQUERY_VERSION')) define('JQUERY_VERSION', '2.2.2.min'); # jquery-1.11.3.min.js
if(!defined('UI_VERSION')) define('UI_VERSION', '1.10.4.custom');
# if(!defined('JQUERY_VERSION')) define('JQUERY_VERSION', ''); # jquery-1.11.3.min.js
# if(!defined('UI_VERSION')) define('UI_VERSION', '');
...
define('INC_BTX', 0); # how js/css will be added to bitrix/ KK : 0 = my, 1 = with $APPLICATION->SetAdditionalCSS()...
...
#            WriteDebugInfo('added menu js code:',$menucode);
    ...
    public static function loadJsDefenitions() {
        ...
        self::$_jslibversions = array(
           'jquery' => ((JQUERY_VERSION>0) ? ('jquery-' . constant('JQUERY_VERSION') .'.js') : '') # KK - свой jQuery + UI!
        ...
        self::$_jsdefs = array(
          ...
          ,'jdmenu' => array(
              'depends' => 'jquery,positionby' # bgiframe deleted (IE6 not supported anymore)!
        ...
        $inc_file = dirname(__FILE__).'/basefunc.upc.php';
        ...
        foreach($md as $jsmodule) {
            ...
            if(isset(self::$_jsdefs[$molo])) { # one of "pre-defined" js libraries
                ...
                if(isset(self::$_jsdefs[$molo]['items'])) {
                  ...
                  if ($molo === 'jquery' && JQUERY_VERSION==='') continue; # Bitrix/KK - already has own jquery
                  if ($molo === 'ui' && UI_VERSION==='') continue; # Bitrix/KK - already has own jquery.ui
                }
...
{ // draw html page designed header, w90 defines percentage width (table tag)
    ...
    if(strpos($hdbody,'<!-- %MENUBLOCK% -->')!==false && function_exists('HtmlBuildMenu')) {
       $brplc['<!-- %MENUBLOCK% -->'] = HtmlBuildMenu(1);
    }

    if(strpos($hdbody,'<!-- %LINKS% -->')!==false) {
...
function MakeHtmlReplacer($PGtitle = '', $ShowTitle = 1, $pgwidth='100%', $rd_url='', $rd_tm=0, $nodraw=0) {
  ...
  $logoimg = defined('LOGOFILE')? LOGOFILE : 'logo.png';
  ...
  if(count(HeaderHelper::$_css_files)) {
      foreach(HeaderHelper::$_css_files as $css) {
          ...
          $cssref = $css . ($myVersion ? (strpos($css,'?')===false?'?':'&').$myVersion : '');
		  if (isset($APPLICATION) && is_object($APPLICATION)) {
            $APPLICATION->AddHeadString("<link rel=\"stylesheet\" type=\"text/css\" href=\"$cssref\" />");

		  }
          else $headBlock .= "<link rel=\"stylesheet\" type=\"text/css\" href=\"$cssref\" />";
      }
  }
  if(HeaderHelper::$html_csscode)  $headBlock .= "<style type=\"text/css\">\n" . HeaderHelper::$html_csscode . "\n</style>\n";

  if(!empty($rd_url)) { // output redirect directive, if redirect url passed
    if(empty($rd_tm)) $rd_tm = 1; // redirect pause time
    $headBlock .= "<meta http-equiv=\"refresh\" content=\"$rd_tm;url=$rd_url\">\n";
  }
  ...
  if(count(HeaderHelper::$_js_files)) {
      ...
      foreach(HeaderHelper::$_js_files as $jsfile) {
          ...
          if(empty($myVersion)) $myVersion = $vers;
          $jsuri = $jsfile . ($myVersion ? (strpos($jsfile,'?')===false?'?':'&').$myVersion : '');
		  if (isset($APPLICATION) && is_object($APPLICATION))
	        $APPLICATION->AddHeadString('<script src="'.$jsuri.'" type="text/javascript"></script>');
	      else
            $headBlock .= "<script type=\"text/javascript\" src=\"$jsuri\"></script>\n";
  ...
  if(!empty($header_jscode['onload']))
      $fulljs .= "jQuery(window).on('load', function() { ".HeaderHelper::$header_jscode['onload']." });\n";


  if(!empty($fulljs)) {
      $headBlock .= "<script type=\"text/javascript\">\n$fulljs\n</script>";
  }
  if ($headBlock) { # adding ALL css/js/redirect in one func, because $APPLICATION->AddHeadScript, $APPLICATION->SetAdditionalCSS sucks !
     if (isset($APPLICATION) && is_object($APPLICATION))
         $APPLICATION->AddHeadString($headBlock,true);
     else echo $headBlock;
  }

  if( (!INC_BTX) && !empty(HeaderHelper::$html_nocache) )
    $ret['<!-- %CACHE% -->'] = '<meta http-equiv="Expires" content="0"><meta http-equiv=pragma content=no-cache>';

  return $ret;
}
# if js/css link has a form plugins/<module>/..., returns version in form "v=X.XX"
...
{ // drawing page footer, including </body></html>
  ...
  $repl = array('<!-- %BACK% -->'=>$baktxt,'<!-- %HOME% -->'=>$hometxt);
...
/* BITRIX, indi - есть такая же функция, конфликтует!
...
function GetMaxUploadSize() {
  ...
  $ret = intval($cfgval);
  if(strpos($cfgval,'M')!==false) $ret *= 1048576;
  elseif(strpos($cfgval,'K')!==false) $ret *= 1024;
  return $ret;
}
/*
function ShowLink($lnk, $txt, $img='', $imgsize=0, $imgheight=0)
{
  global $smpic, $clr2, $clr4, $clrDark;
// echo "<span class=\"box2\"><a href=\"$lnk\"><b>$txt</b></a></span><br><br>\n";
  $himage = '';
  if(!empty($img))
  {
     $sz = empty($imgsize)? '' : "width='$imgsize' " . ($imgheight ? "height='$imgheight'": " height='$imgsize'");
     $himage = "<img src='".IMGPATH."$img' border=0 $sz>&nbsp;";
  }
  echo "<div align=left><a href=\"$lnk\">$himage<b>$txt</b></a></div><br>\n";
}

// AdminNotify - sends Email to admin about some situation
function AdminNotify($ertext, $module='')
{
  global $sysadmin, $PHP_SELF;
  if(empty($module)) $module = $PHP_SELF;
  if(!empty($sysadmin))
  {
            mail($sysadmin, 'ALERT from WWW server',$ertext."\n----\nmodule:$module");
  }

} // AdminNotify() end

function OraConnect($ertext='')
{
  global $ora_logtype, $osess, $ora_user,$ora_psw,$ora_alias;
  $conn = 0;
  if(!empty($ora_user) && !empty($ora_psw) && !empty($ora_alias) )
  {
    ob_start(); // put html header, intercept output to var !
    if(empty($ora_logtype))
      $conn = OciLogon($ora_user,base64_decode($ora_psw),$ora_alias);
    else
      $conn = OciPLogon($ora_user,base64_decode($ora_psw),$ora_alias);
    $osess = $conn;
    $ertext = ob_get_contents();
    ob_end_clean(); //
  }
  else
  {
    $ertext = 'не заданы параметры доступа к базе. Обратитесь к администратору';
  }
  return $conn;
}

function OraDisconnect($conn=0)
{
  global $ora_logtype, $osess;
  if(!empty($ora_logtype)) return;
  if(!empty($conn))  OCILogoff($conn);
  else if(!empty($osess))  OCILogoff($osess);
}
*/
...
function AddToDate($srcdate, $years=0,$months=0,$days=0) {
  ...
  $mlen = $molen[$finmonth];
  $nday = min(intval($nday), $molen[$finmonth]); # корректный переход на 1 месяц с 31 января - не на 03 марта, а на 29/29 февр. (конец м-ца)
  $finday = $nday+$days;
  if($finday>0) {
    $mlen = $molen[$finmonth];
    while($finday>$mlen) {
...
function DiffDays($datefrom, $dateto,$years=false) {
  if(is_numeric($datefrom) && strlen($datefrom)==8) { # yyyymmdd or ddmmyyyy
      ...
      elseif(substr($datefrom,0,4)>=1900 && susbtr($datefrom,-2)<=31) # yyyymmdd
  ...
  if(floatval($dt2[2])>=1000) { # dd.mm.yyyy passed, convert array to y,m,d
    $dt2 = array($dt2[2],$dt2[1],$dt2[0]);
  }
  $sign = 1;
  if($dt1[0]*10000+$dt1[1]*100+$dt1[2] > $dt2[0]*10000+$dt2[1]*100+$dt2[2]) {
...
function isdigit($strg) {
  $first = substr($strg,0,1);
  return (strpos('0123456789',$first)!==false);
}
...
function TextToPrint($txt='') {
  ...
  $ret = $txt;
  if(strpos($ret,"\r",1)) $ret = str_replace("\r",'<br>', $ret);
  elseif(strpos($ret,"\n",1)) $ret = str_replace("\n",'<br>', $ret); //Unix systems
  if(ini_get('magic_quotes_gpc')) {
...
function WriteDebugInfo() {
  ...
  if($numargs==0) { @unlink(Cdebs::$outFile); return; }
  ...
  if(isset ($runinfo[$ilev]['file']))
    $pref = $runinfo[$ilev]['file'].'['.$runinfo[$ilev]['line'].']';
...
function getSubdirs($rootfolder, $mask='',$recursive=false) {
    if(!$mask) $mask = '*.*';
    if(substr($rootfolder,-1)!='/' && substr($rootfolder,-1)!=='\\') $rootfolder .='/';
...
function generateMenu($data, &$menubody, $id='',$crumbs=array()) { # recursive function actually builds menu HTML code
    ...
    if(!$menubody) $menubody = "<ul id=\"$id\" class=\"jd_menu\">";
    else $menubody .= "\n  <ul>";
...
function SmartTextWrap($param, $maxlen=80) {
    ...
    while(strlen($src) > $maxlen) {
        $lfpos = strpos($src,$strlf);
        if($lfpos!== false && $lfpos <= $maxlen) {
           $ret .= substr($src, 0, $lfpos+1);
           $src = substr($src, $lfpos+1);
           continue;
        }
        $fragment = substr($src,0,$maxlen);
        $spacepos = max(strrpos($fragment, ' '), strrpos($fragment, ','), strrpos($fragment, ';')
           ,strrpos($fragment, '.'),strrpos($fragment, '-'),strrpos($fragment, '/'));
        if($spacepos === false) $spacepos = $maxlen;
        $ret .= substr($src, 0, $spacepos+1) . $strlf;
        $src = substr($src, $spacepos+1);
    }
    $ret .= $src;
    return $ret;
}
