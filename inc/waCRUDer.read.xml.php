...
/**
* @package waCRUDer
* @name waCRUDer.read.xml.php
* reading table definition from ".xml" file
*/

class WaCRUDerReadXml extends WaCRUDerReadDef { # reading table definition from "TPL" file

    public function getFileExt() { return 'xml'; }
    public function getFilePrefix() { return 'tdef'; }

    function read($filename,$options = false) { # ex. ParseTplDef($filename, $foredit=0,$ischild=0)
        writeDebugInfo("filename to read ", $filename);
        $ret = new WaTableDef;
        ...
        if(is_file($filename)) {
            ...
            # file_put_contents('_'.$filename.'.read', print_r($xml,1));
            # if (!empty($xml->script)) file_put_contents($filename.'.script', (string)$xml->script);
		}
        elseif (substr($filename,0,5) === '<?xml') {
			$xml = simplexml_load_string($filename);
		}
		else $xml = false;
		if (!$xml) {
            $this->_errormessage = 'Wrong XML Definition file/string or file not found';
            return false;
        }
        if (!empty($xml->parenttables)) $ret->parenttables = explode(',', $xml->parenttables);
        if (!empty($xml->rpp)) $ret->rpp = (string) $xml->rpp;
        if (!empty($xml->browsewidth)) $ret->browsewidth = (string) $xml->browsewidth;

		if (!empty($xml->script)) $ret->script = trim((string)$xml->script,"\r\n");

		foreach($xml->fields->children() as $f) {
			$id = (string) $f['id'];
			$fld = new waFieldDef;
			$fld->id = $id;
			$fld->type = (string) $f['type'];
            if (!empty($f['primary'])) {
                $ret->_pkfields[] = $id;
                $ret->_pkfield = $id;
            }
            $fld->subtype = (!empty($f['subtype']) ? (string) $f['subtype'] : '');
			$fld->_autoinc = empty($f['autoinc']) ? 0 : 1;
			$fld->length = (string) $f['length'];
			$fld->desc = isset($f['desc']) ? (string)$f['desc'] : $fid;
			$fld->shortdesc = !empty($f['shortdesc']) ? (string)$f['shortdesc'] : $fld->desc;
			$fld->notnull = isset($f['notnull']) ? (string)$f['notnull'] : '';
			$fld->defvalue = isset($f['defvalue']) ? (string)$f['defvalue'] : '';
			$fld->showcond = isset($f['showcond']) ? (string)$f['showcond'] : '';
			$fld->showformula = isset($f['showformula']) ? (string)$f['showformula'] : '';
			$fld->editcond = isset($f['editcond']) ? (string)$f['editcond'] : '';
			$fld->edittype = isset($f['edittype']) ? (string)$f['edittype'] : '';
			$fld->editoptions = !empty($f['editoptions']) ? (string)$f['editoptions'] : '';
			$fld->editattribs = !empty($f['editattribs']) ? self::XmlUnserialize($f['editattribs']) : '';

			$ret->fields[$id] = $fld;
		}
		$ret->canview =$ret->canedit = $ret->candelete = $ret->caninsert = 1;
        return $ret;
        #TODO: implement rest fileds...

        foreach($xml->children() as $id=>$item){ #<3>
            echo "<ptr>xml item $id" . print_r($item,1). '</pre>';
            /***
            if($b_comment) {
        ...
        #        if(count($tar)<2 && $tar[0]) continue;
            ...
            case 'DERIVEDFROM': case 'PARENTTABLES': # get parent table(s) def.
              ...
              break;
            case 'CHARSET':
              $ret->charset = strtoupper($tar[1]);
              if($ret->charset=='UTF8') $ret->charset = 'UTF-8';
              break;
            case 'COLLATE':
              $ret->collate = strtoupper($tar[1]); # Cyrillic UTF: utf8_general_ci
              if($ret->charset=='UTF8') $ret->charset = 'UTF-8';
              break;
            case 'ID':
        ...
        #          if($fld->type=='TINYINT' && !$fld->type) $fld->type=1;
              ...
              if (count($_arr)>1) $fld->editoptions = $_arr; # SELECT,@funs,formats,... => editoptions = ['@func', formats,...]
        ...
        #          $fld->descending = !empty($tar[4]);
            ...
            case 'FULLTEXT': case 'FTINDEX':
              $idxname = count($tar)>2 ? trim($tar[1]) : ('ft_'.$ret->$id.(count($ret->ftindexes)+1) );
	...
	/**
	* decode "<", ">", ["] and ['] from "/xNN" representaions
	*
	* @param mixed $src
	*/
	public static function XmlUnserialize($src) {
		return str_replace(
