...
class Stub {
    ...
    public function request($requestString, $arHist=[], $context='') {
        ...
        if(mb_strtolower($requestString, 'UTF-8') == 'абракадабра' && class_exists('\textgen')) {
            ...
            $outFiles = \plugins\chatbot\aibus::getOutputFiles();
            if(is_array($outFiles) && count($outFiles)) {
                foreach($outFiles as $item) {
                    $answer .= "\n* Вы приложили файл: $item[name]";
                }
                \plugins\chatbot\aibus::clearOutputFiles(); # типа я их обработал
            }
    ...
    public static function getStartMessage() {
        return 'У меня нет ИИ, и вообще никакого И, это просто заглушка для отладки модулей чат-бота.<br>'
         . 'Введите @test, или @test 2 для вызова тестовых шаблонов ответов в формате MarkDown';
    }
    # сделать markdown ответ
    public function modelList($params = FALSE) {
        $txtOut = "## Список моделей в STUB\r\n"
         . "|No|Модель|Описание|\r\n|---|---|---|\r\n";
        $txtOut .= "| 1 | blablabla | Чухня полная (зато даром)|\r\n";
        $txtOut .= "| 2 | abrakadabra | Чухня не просто полная, а ваще трындец (эта дорогая - 100500 щелбанов за тыщу токенов)|\r\n";
        return $txtOut;
    }
