...
* @version 0.08.001
* modified 2025-10-14 / created 2025-02-26
*/
class BindClient {
    static $debug = 0;
    static $initOldDays = 370; # initClientList() - при начальном заполнении базы клиентов брать полисы не старее NNN дней

    public const ERR_NO_ACCESS = 1001; # у запросившего данные клмента нет к нему доступа
...
EOHTM;
        ...
        if(empty($mode))  { # AJAX вызов с формы калькулфтора или agredit - нужен только код формы поиска
...
EOHTM;
    ...
    */
    public static function getMyClients() {
        ...
        if(!empty($fam)) $wcond[] = "imia LIKE '$imia%'";
        if(!empty($fam)) $wcond[] = "otch LIKE '$otch%'";
        ...
        if(is_array($cliData) && count($cliData)) {
            ...
            foreach($cliData as $row) {
                $listBody .= "<tr><td id=\"found_{$row['id']}\">$row[fam] $row[imia] $row[otch], ".to_char($row['birth'])
                  ."</td><td><input button class='btn btn-primary' onclick='fcli.setClient($row[id])' value='Выбрать'/></td></tr>";
            }
            $listBody .= "</table>";
        }
    ...
    */
    ...
    public static function checkClientUpdates($bkObj, $clientid, $birthFldName='', $return=FALSE) {
        ...
        if($birthValue !== $cliData['birth']) {
            $goodBirth = to_char($cliData['birth']);
            if($return) return FALSE;
            exit('1'
              . AjaxResponse::setValue($newBirthFld, $goodBirth)
              . AjaxResponse::showError('Нельзя менять дату рождения клиента!<br>(возвращена к исходному значению)')
            );
