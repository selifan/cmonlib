...
class FlexReport {
    ...
    public function loadConfig($cfgParam) {
        ...
        if (is_array($cfgParam)) {
            # ready-to-use array passed. Just get it and return.
            $this->_cfg = $cfgParam;
            return;
        }
        $xml = FALSE;
    ...
    */
    public function execute($params = FALSE) {
        ...
        if (!empty($this->_cfg['query'])) {
            ...
            # exit("report SQL:<br>" . \appEnv::$db->getLastQuery());
        }
    ...
    private function _reportHeaders() {
        ...
        $title = $this->evalParam($this->_cfg['title'] ?? 'Отчет');
