...
class Astedit
{
    ...
    {
        if (is_object($anotherDb)) {
            self::$dn = &$anotherDb;
        } else {
            global $as_dbengine;
            if (!is_object(self::$db)) {
                if (class_exists("AppEnv") && isset(AppEnv::$db)) {
                    self::$db = &AppEnv::$db;
                } elseif (is_object($as_dbengine)) {
                    self::$db = &$as_dbengine;
                } elseif (class_exists("CDbEngine")) {
                    self::$db = CDbEngine::getInstance();
                }
            }
        }
    }

    public static function getCommonJsCode()
    {
        global $ast_tips;
...
asteditJs = {
  ...
  delTitlew: '$delTitle',
  deletingId: false,
  astDoAction: function(tid, action, recid) {
    // alert(action + ' '+recid);
    asteditJs.workingId = tid;
    console.log(tid, action, recid);
    switch(action) {
      case 'edit':
        var gotoUrl = asteditJs.tc[tid].postUrl + "&ast_act=edit&id="+recid;
        // TODO: inpage ajax editing!
        console.log(gotoUrl);
        window.location = gotoUrl;
        break;
      case 'update':
        asteditJs.updateData();
        break;
      case 'delete':
        asteditJs.deletingId = recid;
        var delPrompt = asteditJs.tc[tid].delPrompt.replace('{id}', recid);
        var delTitle = asteditJs.tc[tid].delTitle;
        dlgConfirm({title: delTitle, text: delPrompt}, asteditJs.performDelete)
        break;
      case 'add':
        var gotoUrl = asteditJs.tc[tid].postUrl + "&ast_act=add&id="+recid;
        console.log(gotoUrl);
        window.location = gotoUrl;
        break;
      case 'cloneRecord':
        var params = { "ast_act":"askCloneRecord", "recid": recid };
        $.post(asteditJs.tc[tid].postUrl, params, function(response) {
  ...
  updateData: function() {
    ...
    var tid = asteditJs.workingId;
    var formData = new FormData($("#astedit_"+tid)[0]);
    $.ajax({
        type: "POST",
        url: asteditJs.tc[tid].postUrl,
        cache: false,
        contentType: false,
        processData: false,
        data: formData,
        success: function(response){
          var tid = asteditJs.workingId;
          if(response=='1') window.location = asteditJs.tc[tid].postUrl;
          else showMessage('Ошибка!', response, 'msg_error');
        },
  ...
  performDelete: function() {
    var tid = asteditJs.workingId,
      postUrl = asteditJs.tc[tid].postUrl,
      params = {'ast_act':'delete', 'id': asteditJs.deletingId};
    console.log("tid:",tid, "del record:", asteditJs.deletingId, " Datatable object:", asteditJs.tc[tid].dTable);
    // return;
  ...
  checkUniqueness: function(tableid, obj, recid){
    ...
    asJs.sendRequest(asteditJs.tc[tableid].postUrl, params, true);
  },
  reloadFirstPage: function(tid) {
    if(!tid) tid = asteditJs.workingId;
    asteditJs.tc[tid].dTable.page(1).draw();
  },
  reloadCurrentPage: function(tid) {
    if(!tid) tid = asteditJs.workingId;
    asteditJs.tc[tid].dTable.draw();
  },
  ...
  resetFilter: function(tid,formid) {
    asteditJs.workingId = tid;
    var params = $("#"+formid).serialize() + "&ast_act=resetfilter";
    $("input.resetfilter", "#"+formid).prop("disabled", true);
    asJs.sendRequest(asteditJs.tc[tid].postUrl, params, true, false, asteditJs.reloadFirstPage);
  },
  showHelp: function(tid) {
    $.post(asteditJs.tc[tid].postUrl, {'asteditajax':1,'ast_act':'showhelp'}, function(response) {
...
EOJS;
    ...
    {
        return "<div class=\"alarm\">$text</div>";
    }
    ...
    public static function localize($id, $default = "")
    {
        if (is_callable("WebApp::getLocalized")) {
            return WebApp::getLocalized($id, $default);
        }
    ...
    public static function upgradeTables($tbar, $tovar = false)
    {
        ...
        if (is_file(__DIR__ . "/astedittsu.php")) {
            ...
            return "module astedittsu.php not found. Upgrade failed";
        }
        $ret = "";
        ...
        foreach ($tbar as $tb) {
            ...
            if (is_array($arr) && count($arr) > 0) {
                ...
                foreach ($arr as $qryname => $qrytext) {
                    #<4>
            ...
            #<2>
                ...
                $ret .= " " . Astedit::localize("no_changes") . "<br>";
            }
        } #<2>
        if (empty($tovar)) {
            echo $ret;
    ...
    public static function dropTables($tbar, $tovar = false)
    {
        # $tbar - Table names array, to check and create/update structures
    ...
    public static function convertTable($tbar, $cset, $ignore = false)
    {
        ...
        foreach ($tbar as $tb) {
            ...
            if (!empty($tbl->id)) {
                self::$db->sql_query(
                    "ALTER TABLE {$tbl->id} CONVERT TO CHARACTER SET $cset"
                );
                $errno = self::$db->sql_errno();
                $result = $errno ? self::$db->sql_error() : " OK";
    ...
    public static function changeCharSet($tbar, $cset, $tovar = false)
    {
        ...
        foreach ($tbar as $tb) {
            ...
            if (!empty($tbl->id)) {
                self::$db->sql_query(
                    "ALTER TABLE $tbl->id DEFAULT CHARSET $cset"
                );
    ...
    public static function viewShortText($txt, $maxlen = 40)
    {
        ...
        $cr = strpos($txt, "\r");
        if ($cr > 0 && $cr < 4 && $cr < $maxlen) {
            $cr = strpos($txt, "\r", $cr + 2);
        }
        if ($cr > 0) {
    ...
    public static function BrShowBool($val = false)
    {
        ...
        } # ui-state-default - add borders
        return empty($val)
            ? ""
            : '<img src="' . IMGPATH . 'btCheck.png" width=18 height=18 />';
    }
    ...
    public static function decodeSqlError($errno, $errText, $tableiId = "")
    {
        ...
        if (!empty($tableiId)) {
            $sRet = str_replace("{tablename}", $tableiId, $sRet);
        }
        return $sRet;
    }
    ...
    public static function evalValue($param, $par_arr = null, $par2 = null)
    {
        if (empty($param)) {
            return false;
        }
        ...
        if ("@" === substr($param, 0, 1)) {
            ...
            if (is_callable($fnc)) {
                $ret = call_user_func($fnc, $par_arr, $par2);
            } else {
                $splt = explode("::", $fnc);
                if (count($splt) > 1 && class_exists($splt[0])) {
                    $clsName = $splt[0];
                    $obj = $splt[1];
                    $evaluated = $clsName::$obj($par_arr, $par2);
                    return $evaluated;
                }
            }
        }
    ...
    {
        $htmRet =
            "<div style='display:flex, justify-content: center; margin:auto'><div style='width:{$width};margin:auto'>" .
            $htmlCode .
            "</div></div>"; # width limiting div
        return $htmRet;
    }
} # astedit end
if (!defined("DEFAULT_TBLTYPE")) {
    define("DEFAULT_TBLTYPE", "MYISAM");
} # default table type for CREATE TABLE operation
...
if (!class_exists("CDbEngine")) {
    @require_once $ast_libpath . "dbaccess.pdo.php"; # DB access wrapper class
}
...
if (!isset($as_cssclass)) {
    $as_cssclass = [];
}
if (empty($as_cssclass["textfield"])) {
    $as_cssclass["textfield"] = "form-control d-inline w-auto-custom";
}
if (empty($as_cssclass["trowhead"])) {
    $as_cssclass["trowhead"] = "head";
}
if (empty($as_cssclass["tdhead"])) {
    $as_cssclass["tdhead"] = "head";
}
#if(empty($as_cssclass['troweven'])) $as_cssclass['troweven'] = 'even';
if (empty($as_cssclass["trowodd"])) {
    $as_cssclass["trowodd"] = "odd";
}
if (empty($as_cssclass["trmouseover"])) {
    $as_cssclass["trmouseover"] = "mouseover";
}
if (empty($as_cssclass["pagelnk"])) {
    $as_cssclass["pagelnk"] = "pagelnk";
}
if (empty($as_cssclass["pagelnka"])) {
    $as_cssclass["pagelnka"] = "pagelnka";
}

if (empty($as_cssclass["button"])) {
    $as_cssclass["button"] = "btn btn-primary";
}
if (!isset($ast_hideserachbar)) {
    $ast_hidesearchbar = false;
} # enable show/hide search toolbar
$ast_datarow = []; # global var will hold data row array
$ast_frmfunctions = []; # registered drawing form field functions for astedit
# You can prepare localized string vars ast_* and include them before this line
if (!isset($ast_tips) or !is_array($ast_tips)) {
    $ast_tips = [];
} # localized interface here !
# titles for edited fields, fill this array only for needed fields ['table']['field']
if (!isset($ast_lang)) {
    $ast_lang = "ru";
} # Set Your language ID here or before
if (is_file("./i18n/" . $ast_lang . "/strings-astedit.php")) {
    @include "./i18n/" . $ast_lang . "/strings-astedit.php";
} else {
    @include_once "astedit-lang.$ast_lang.php";
}
if (
    !empty($ast_tips["charset"]) &&
    defined("MAINCHARSET") &&
    constant("MAINCHARSET") != ""
) {
    if ($ast_tips["charset"] !== MAINCHARSET) {
        mb_convert_variables(MAINCHARSET, $ast_tips["charset"], $ast_tips);
    }
}

if (!isset($ast_tips["clone_record"])) {
    $ast_tips["clone_record"] = "Clone record";
}
if (!isset($ast_tips["tipclone"])) {
    $ast_tips["tipclone"] = "Clone record";
}
if (!isset($ast_tips["table_frozen"])) {
    $ast_tips["table_frozen"] = "Table is frozen!";
}
if (!isset($ast_tips["tipadd"])) {
    $ast_tips["tipadd"] = "Add";
}
if (!isset($ast_tips["tipedit"])) {
    $ast_tips["tipedit"] = "Edit";
}
if (!isset($ast_tips["tipdelete"])) {
    $ast_tips["tipdelete"] = "Delete";
}
if (!isset($ast_tips["tiphelpedit"])) {
    $ast_tips["tiphelpedit"] = "Show some help for this page";
}
if (!isset($ast_tips["help_page_notfound"])) {
    $ast_tips["help_page_notfound"] = "Help page not found.";
}
if (!isset($ast_tips["help"])) {
    $ast_tips["help"] = "Help";
}
...
if (!isset($_SESSION)) {
    @session_start();
} # session engine is widely used, so start it!
# intercept 'sort' command, to set cookies before any output...

$ast_parm = array_merge($_GET, $_POST);

if (!empty($ast_parm["ast_act"])) {
    $ast_act = $ast_parm["ast_act"];
}

class CFieldDefinition # one table field definition
{
    ...
    public function GetDefaultValue()
    {
        ...
        if ($def !== "" && $def[0] === "'") {
            $def = trim($def, "'");
        } elseif ($def != "" && ($def[0] === "@" || $def[0] === "#")) {
            $def = astedit::evalValue($def);
        } elseif ($this->type == "DATE" || $this->type == "DATETIME") {
            if (in_array($def, ["now", "{now}"])) {
                $def = date("Y-m-d H:i:s");
            } elseif (in_array($def, ["today", "{today}"])) {
                $def = date("Y-m-d");
            } else {
                $def = "";
            }
...
class CIndexDefinition # one table field definition
{
...
class CTableDefinition # class for holding info about table structure
{
    const DEFAULTDELIMS = "/[,; ]/";
    static $ast_ajaxfnc_drawn = false;
    static $ast_tableprefix = "";
    public static $dataRow = false;
    private static $tplFolder = [""];
    static $tplMerged = false;
    public $tplfile = ""; # loaded definition filename (w/o .tpl)
    private $request = [];
    private $debug = 0;
    ...
    public $browsefilter = []; # filter for browse mode
    ...
    public $browseHeight = ""; # datatables new: browsing area height
    ...
    public $parenttables = []; # internal, holds parent table names list
    public $fields = []; # field list (CFieldDefinition)
    public $reports = [];
    protected $extfields = [];
    protected $extensions = [];
    private $errUpdate = false;
    public $indexes = []; # all indexes in the table
    public $customcol = []; # additional columns (hrefs, images etc)in Browse:
    public $viewfields = []; # fields to view in browse screen, can be overriden by SetView()
    public $events = []; # editing-time javascript events for some data types: ['date.onClick']='DateRepair(this)'
    ...
    public $ftindexes = []; # FULLTEXT|f1,f2... sets FULLTEXT index for the table
    ...
    public $_wysiwyg = []; # all WYSIWYG-edited fields will be listed here
    ...
    public $_converters = []; # ['myfield'] = 'FieldFuncConvertor'
    ...
    public $_browseheadertags = []; # user <TD> tags for the header line
    ...
    public $_browsetags = []; # each column can have specific tags in the browse grid
    ...
    public $reset_chain = []; # reset current filters chains when "parent" filter is changed
...
*/
    ...
    function SetWysiwygType($par = "")
    {
        $GLOBALS["ast_wysiwyg_type"] = strtolower($par);
    }
    function AllFieldsUpdatable($par = true)
    {
        $this->_updt_all = $par;
    }
    ...
    function ParseTplDef($filename, $foredit = 0, $ischild = 0)
    {
        ...
        if (count($splitted) == 1) {
            ...
            $this->id = $splitted[count($splitted) - 2];
        }
        $b_comment = 0;
        if (is_file($filename)) {
            $lst = @file($filename, 1);
        ...
        if (!is_array($lst) or count($lst) < 1) {
            $this->_errormessage =
                "Definition file not found or has no fields or read error";
            return false;
        }
        $fieldNo = 0;
        foreach ($lst as $lineno => $strkFull) {
            #<3>
            ...
            if (in_array(strtolower($strk), ["<script>", "<js>"])) {
                $this->_inJs = true;
                continue;
            }
            if (in_array(strtolower($strk), ["</script>", "</js>"])) {
            ...
            if (substr($strk, 0, 1) == "#") {
                continue;
            } elseif (substr($strk, 0, 2) == "/*") {
                $b_comment = 1;
                continue;
            } elseif (substr($strk, -2) === "*/") {
                $b_comment = 0;
                continue;
            }
            ...
            if ($key === "IF") {
                # Conditional declaration: IF|{EXPRESSION}|FIELD... etc.
                array_shift($tar);
                if (count($tar) < 3) {
                    contiue;
                } # wrong IF line
            ...
            switch ($key) {
                ...
                case "RPP":
                    ...
                    break;
                case "DERIVEDFROM":
                case "PARENTTABLES": # get parent table(s) def.
                ...
                case "CHARSET":
                    ...
                    if (strtoupper($this->charset) == "UTF8") {
                        $this->charset = "UTF-8";
                    }
                    break;
                ...
                case "DATACHARSET":
                    ...
                    if (strtoupper($this->datacharset) == "UTF8") {
                ...
                case "COLLATE":
                    ...
                    break;
                case "ID":
                    $this->id = str_ireplace(
                        Astedit::PREFIX_MACRO,
                        self::$ast_tableprefix,
                        trim($tar[1])
                    );
                    if ($this->tbrowseid === "") {
                        $this->tbrowseid = $this->id;
                    }
                    break;
                case "BROWSEID":
                case "VIEWID":
                    $this->tbrowseid = str_ireplace(
                        Astedit::PREFIX_MACRO,
                        self::$ast_tableprefix,
                        trim($tar[1])
                    );
                ...
                case "BRFILTER":
                    if (substr($tar[1], 0, 1) === "@") {
                        $func = substr($tar[1], 1);
                        if (is_callable($func)) {
                            $oneFilter = call_user_func($func, $this->id);
                        } else {
                            $oneFilter = false;
                        }
                    } else {
                ...
                case "ORDERBY":
                    $this->browseorder .=
                        ($this->browseorder == "" ? "" : ",") . $tar[1];
                    break;
                case "BRWIDTH":
                case "GRIDWIDTH":
                    $this->browsewidth = $tar[1];
                    if (defined("IF_LIMIT_WIDTH")) {
                        # restrict maximal grid width, if IF_LIMIT_WIDTH set
                        if (strpos($this->browsewidth, "%") === false) {
                            $this->browsewidth = min(
                                $this->browsewidth,
                                constant("IF_LIMIT_WIDTH")
                            );
                        }
                    }
                    if (is_numeric($this->browsewidth)) {
                        $this->browsewidth .= "px";
                    }
                    break;

                case "BRHEIGHT":
                case "GRIDHEIGHT": # grid height
                    $this->browseHeight = $tar[1];
                    if (is_numeric($this->browseHeight)) {
                        $this->browseHeight .= "px";
                    }
                    break;
                ...
                case "EDITFORMWIDTH":
                    ...
                    if (defined("IF_LIMIT_WIDTH")) {
                        ...
                        if (strpos($this->editformwidth, "%") === false) {
                            $this->editformwidth = min(
                                $this->editformwidth,
                                constant("IF_LIMIT_WIDTH")
                            );
                        }
                    }
                    if (is_numeric($this->editformwidth)) {
                        $this->editformwidth .= "px";
                    }
                    break;
                case "LOADJSCODE":
                    ...
                    } elseif (is_file($loaderFn)) {
                        $this->_jscode .= file_get_contents($loaderFn);
                    }
                    break;
                ...
                case "BLISTHEIGHT":
                    ...
                    if ($this->blist_height == "") {
                        $this->blist_height = "100px";
                    }
                    break;
                case "BLISTFULLFORM":
                case "BFF":
                ...
                case "BROWSESTYLES":
                    ...
                    foreach ($flds as $fldid) {
                        if (!empty($fldid)) {
                            if (!isset($tar[2]) || empty($tar[2])) {
                                unset($this->_browsetags[$fldid]);
                            } else {
                                $this->_browsetags[$fldid] = trim($tar[2]);
                            }
                        }
                    }
                case "RECDELETEFUNC":
                ...
                case "EDITTEMPLATE":
                    ...
                    if (!empty($etplFname)) {
                        if (!is_file($etplFname)) {
                            $tdir = dirname($this->tplfile);
                            if (is_file("$tdir/$etplFname")) {
                                $etplFname = "$tdir/$etplFname";
                            } else {
                                $etplFname = "";
                            }
                        }
                        if ($etplFname) {
                            $this->edit_template = $etplFname;
                        }
                    }
                    break;
                case "EDITSUBFORM":
                ...
                case "CLONABLE":
                    ...
                    if (!empty($tar[4])) {
                        $this->confirm_clone_child = $tar[4];
                    }
                ...
                case "EXTFIELD": # External Field (stored outside of this table!
                    ...
                    if ($fld->type === "NUMBER") {
                        # Oracle 'NUMBER' - to INT or DECIMAL (MySQL)
                        $splen = explode(",", $fld->length);
                        $fld->type =
                            count($splen) > 1 && $splen[1] > 0
                                ? "DECIMAL"
                                : "INT";
                    }
                    ...
                    if (
                        ...
                        $fld->length = "";
                    } # error length protect
                    $fld->notnull = !empty($tar[7]) || $notNull;
                    $fld->defvalue = isset($tar[8]) ? $tar[8] : "";
                    $fld->showcond = empty($tar[9])
                        ? ""
                        : astedit::evalValue($tar[9]);
                    ...
                    if ($fld->showformula !== "") {
                        ...
                        $fld->showattr = $splt; # [0]=text-align,[1]-bgcolor(may be @function),[2]-css class in grid, [4] - class
                    }
                    ...
                    $fld->edittype = empty($trimmedType)
                        ? "TEXT"
                        : $trimmedType;
                ...
                case "PRIMARYKEY":
                case "PK": # fields in PRIMARY KEY() list
                ...
                case "CUSTOMCOLUMN": # non-field columns in browse page #ex-BRCUSTOMHREF
                    ...
                    if (!empty($htmlcode)) {
                        $htmlcode = astedit::evalValue($htmlcode, $this->id);
                    }
                ...
                case "REPORT":
                    $rep_id = !empty($tar[1])
                        ? $tar[1]
                        : "report" . (count($this->reports) + 1);
                    $rep_fields = $tar[2] ?? "";
                    $rep_filter = $tar[3] ?? "";
                    $rep_label = $tar[4] ?? "";

                    if (!empty($rep_label)) {
                        $rep_label = astedit::evalValue($rep_label, [
                            $this->id,
                            $rep_id,
                        ]);
                    }
                    $rep_title = $tar[5] ?? "";
                    if (!empty($rep_title)) {
                        $rep_title = astedit::evalValue($rep_title, [
                            $this->id,
                            $rep_id,
                        ]);
                    }
                    ...
                    if (is_callable("asteditReport::getReportButtonHtml")) {
                        $reportButton = asteditReport::getReportButtonHtml(
                            $this->id,
                            $rep_id,
                            $rep_label,
                            $rep_title
                        ...
                        $reportButton =
                            "<input type=\"button\" id=\"$rep_id\" class=\"btn\" value=\"$rep_label\" " .
                            "onclick=\"asteditReport.run('$this->id','$rep_id')\" $rTitle/>";
                    }
                    $this->addToolbarElement($reportButton);
                ...
                case "TOOLBAR": # HTML code for toolbar (buttons etc.)
                    ...
                    if ($htmlcode) {
                        $this->addToolbarElement(
                            astedit::evalValue($htmlcode, $this->id)
                        );
                    }
                    break;
                ...
                case "MULTISELECT":
                    ...
                    $this->_multiselectFunc = isset($tar[1])
                        ? trim($tar[1])
                        : "";
                    break;
                case "TYPE":
                ...
                case 'EXTENSION': # load astedit extension
                    $extName = strtolower($tar[1] ?? "");
                    if (!empty($extName)) {
                        $extFilename = __DIR__ . "/astedit.ext.$extName.php";
                        if (!is_file($extFilename)) {
                            throw new Exception(
                                "Extension module $extFilename not found"
                            );
                        }
                        $extClassName = "astedit\\" . $extName;
                        include_once $extFilename;
                        $addParams = $tar;
                        array_shift($addParams);
                        array_shift($addParams);
                        $extObjectId = $addParams[0] ?? 'noname';
                        if (class_exists($extClassName)) {
                            $this->extensions[] = new $extClassName(
                                $this,
                                $addParams
                            );
                        }
                        astedit::$extensionParams["$extName-{$this->id}-$extObjectId"] = $addParams;
                    }
                    break;
            } #<4> switch
        } #<3>
        if ($this->datacharset == "") {
            $this->datacharset = $this->charset;
        }
        # auto conect online Help if file exist
        $helpFile = dirname($this->tplfile) . "/" . $this->id . ".help.htm";
        if (is_file($helpFile)) {
            $this->helppage = 1;
        }

        # auto conect form if file exist
        $formFile = dirname($this->tplfile) . "/" . $this->id . ".form.htm";
        if (is_file($formFile) && empty($this->edit_template)) {
            $this->edit_template = $formFile;
        }
        # Add "externally added dependensies"
        if (
            class_exists("AppEnv") &&
            is_callable("AppEnv::getTableDependencies") &&
            ($deps = AppEnv::getTableDependencies($this->id))
        ) {
    ...
    public function addToolbarElement($htmlCode)
    {
    ...
    function AddField(
        ...
        if ($fld->type == "PKA" || $fld->type == "PK") {
            #<3>
            $fld->type = "BIGINT"; #
            $fld->length = 20;
            if (!$this->_pkflistset) {
                $this->_pkfields[] = $fld->id;
            }
        } #<3>
        ...
        $this->fields[$fldid] = $fld;
        if (astedit::evalValue($fld->showcond)) {
            $this->viewfields[] = $fldid;
        }
        return true;
    }
    ...
    function SetLinkedPictures(
        ...
        if (class_exists("cimagemanager")) {
            $this->_picobj = new CImageManager([
                "tablename" => $tablename,
                "picfolder" => $subfolder,
                "width" => $width,
                "height" => $height,
        ...
        if (is_object($this->_picobj)) {
            if ($viewscript) {
                $this->_picobj->SetViewScript($viewscript);
            }
    ...
    function ReadDefinition($fileName, $csetTo = "", $foredit = 0, $ischild = 0)
    {
        ...
        $this->tplfile = $src_file = $fileName;
        $folders = self::$tplFolder;
        $inMapFile = false;
        if (class_exists("AppEnv")) {
            if (
                isset(AppEnv::$tplMappings) &&
                is_array(AppEnv::$tplMappings) &&
                isset(AppEnv::$tplMappings[$fileName])
            ) {
        ...
        if (function_exists("AstStructModifier")) {
            AstStructModifier($this);
        }
        $this->filename = $fileName;
        foreach ($this->fields as $fld) {
            if (astedit::evalValue($fld->showcond)) {
                $this->viewfields[] = $fld->id;
            }
        }
        if ($csetTo == "") {
            $csetTo = MAINCHARSET;
        ...
        if (empty($foredit) && count($this->parenttables)) {
            foreach ($this->parenttables as $parentname) {
                ...
                foreach ($parent->fields as $fid => $f) {
                    ...
                    $this->fields[$fid] = $f;
                    if (astedit::evalValue($f->showcond)) {
                        $this->viewfields[] = $f->id;
                    }
                }
                if (count($parent->_pkfields)) {
                    $this->_pkfields = array_merge(
                        $this->_pkfields,
                        $parent->_pkfields
                    );
                }
                $this->childtables = array_merge(
                    $this->childtables,
                    $parent->childtables
                );
                $this->indexes = array_merge($this->indexes, $parent->indexes);
                $this->customcol = array_merge(
                    $this->customcol,
                    $parent->customcol
                );
                $this->ftindexes = array_merge(
                    $this->ftindexes,
                    $parent->ftindexes
                );
                if (empty($this->rowclassfunc)) {
                    $this->rowclassfunc = $parent->rowclassfunc;
                }
            }
        }
    ...
    public static function setTplFolder($path)
    {
        self::$tplFolder = is_array($path) ? $path : explode(",", $path);
    }
    ...
    public static function addTplFolder($path)
    {
        ...
        if (is_array($path)) {
            foreach ($path as $item) {
                ...
                if (!in_array($item, self::$tplFolder)) {
                    self::$tplFolder[] = $item;
                }
            }
        }
    ...
    public function __construct(
        ...
        if (defined("IF_LIMIT_WIDTH") && constant("IF_LIMIT_WIDTH") < 900) {
            # restrict maximal grid width, if IF_LIMIT_WIDTH set
            $this->searchcols = 1;
        }
        ...
        if (!defined("IMGPATH")) {
            define("IMGPATH", "img/");
        } # path to all button pics and other graphics
        if (defined("ADJACENTLINKS")) {
            $this->_adjacentLinks = intval(constant("ADJACENTLINKS"));
        }
        ...
        if (
            class_exists("AppEnv") &&
            property_exists("AppEnv", "TABLES_PREFIX")
        ) {
            self::$ast_tableprefix = AppEnv::TABLES_PREFIX;
        } elseif (defined("AST_TABLEPREFIX")) {
            self::$ast_tableprefix = constant("AST_TABLEPREFIX");
        }
    ...
    function SetAjaxMode($ajax = 1)
    {
        $this->ajaxmode = $ajax;
    }

    function SetViewMode($par = true) {
        $this->_viewmode = $par;
    }
    ...
    function RegisterFunction($regtype, $fieldtype, $funcname)
    {
        ...
        if (empty($regtype)) {
            $regtype = AST_FORM_ELEMENT;
        }
        $fieldtype = strtoupper($fieldtype);
        if (!isset($ast_frmfunctions[$regtype])) {
            $ast_frmfunctions[$regtype] = [];
        }
    ...
    function drawSearchBar()
    {
        ...
        $txtclass = empty($as_cssclass["textfield"])
            ? ""
            : "class='{$as_cssclass["textfield"]}'";
       ...
       <table class='table mx-auto w-auto border-primary table-light'>\n"; # astedit-search
        ...
        foreach ($searchcoll as $isc => $item) {
            ...
            if (substr($fname, 0, 1) == "@") {
                ...
                if (is_callable($fltfunc)) {
                    $searchcode =
                        "<input type='hidden' name='search_function' value='$fltfunc' />" .
                        call_user_func($fltfunc, "form");
                }
            } else {
                #<4> draw standard search form for a field
                $ffunc = "";
                ...
                $searchHtml = "<input type=\"hidden\" name=\"flt_name\" value=\"$fname\" />";
                if ($sr_type === "range") {
                    $search1 = $this->drawInput($fname, "from", "", $values);
                    $search2 = $this->drawInput($fname, "to", "", $values);
                    $lb_from = is_callable("AppEnv::getLocalized")
                        ? AppEnv::getLocalized("label_range_from", "")
                        : " ";
                    $lb_to = is_callable("AppEnv::getLocalized")
                        ? AppEnv::getLocalized("label_range_to", "...")
                        : "...";
                    $searchHtml .=
                        $fld->shortdesc .
                        ": " .
            ...
            $formCode =
                "<td class='nowrap'><form id='$formid' onsubmit='return false'>" .
                " <table><tr><div class='d-flex justify-content-end'><div>$searchcode</div>&nbsp;" .
                " <input type='button' class=\"$btnclass setfilter\" name='setfilter' value='{$ast_tips["setfilter"]}' onclick=\"asteditJs.setFilter('$this->tbrowseid','$formid')\" />";

            if (true) {
                # ($showreset) add 'RESET' mini-form (one button !)
                if ($fltfunc) {
                    $fld_id = "@$fltfunc";
                    $disabl = call_user_func($fltfunc, "isfilterset")
                        ? ""
                        : 'disabled="disabled"';
                } elseif (is_object($fld)) {
                    $fld_id = $fld->id;
                    $disabl = isset($_SESSION[$tid][$fld->id])
                        ? ""
                        : 'disabled="disabled"';
                }
                $formCode .= "&nbsp;&nbsp;<input type='button' class=\"$btnclass resetfilter\" value=\"{$ast_tips["tipresetfilter"]}\" $disabl onclick=\"asteditJs.resetFilter('$this->tbrowseid','$formid')\" />";
            }
            $formCode .= "</div></tr></table></form></td>\n";
        ...
        if ($rowstarted) {
            $sbody .= "</tr>";
        } # vaild table row ending !
        $sbody = $sbody . "</table>\n<!-- search bar end -->\n";
        if ($this->_togglefilters) {
            $state = 0; # !empty($_COOKIE['ast_hidesearch']);
            $srdisp = "show"; # ($state)? 'show':'';
            $tgltitle = isset($ast_tips["title_showsearchbar"])
                ? $ast_tips["title_showsearchbar"]
                : "Show/hide search";
      ...
      <div class='accordion-item'>
        <h2 class='accordion-header' id='panelsStayOpen-headingOne' onclick=\"ToggleSearchBar()\" title='$tgltitle'>
          <button id='search_toggle' class='accordion-button' type='button' data-bs-toggle='collapse' data-bs-target='#tr_astsearchbar' aria-expanded='$state' aria-controls='tr_astsearchbar'>
    ...
    function GetFieldNo($nName)
    {
        # GetFieldNo
        if (isset($this->fields[$nName])) {
            return $nName;
        }
        return -1;
    } # GetFieldNo
    ...
    {
        global $ast_datarow;
        if (!is_array($rowdata) && !empty($ast_datarow)) {
            $rowdata = $ast_datarow;
        }
        $arr = [];
        foreach ($this->_pkfields as $onekey) {
            $arr[] = isset($rowdata[$onekey]) ? $rowdata[$onekey] : "";
        }
    ...
    {
        $valar = is_array($values)
            ? $values
            : explode(AST_PKDELIMITER, $values);
        $ret = [];
    ...
    function SetBrowseTags($fldid, $tags = "")
    {
        $this->_browsetags[$fldid] = $tags;
    }
    ...
    */
        ...
        $pkvalue = count($this->_pkfields)
            ? $this->FullPkFieldAsString($arRow)
            : "xx";
        ...
        foreach ($this->viewfields as $fno => $fid) {
            $tdid = $fid . $pkvalid;
            $tdtags = isset($this->_browsetags[$fid])
                ? $this->_browsetags[$fid]
                : "";
            ...
            $val = $fldValue[$fid] ?? $arRow[$fid] ?? "";

            $cellClass = "";
            if (empty($tdtags) && is_object($fdef)) {
                $tdstyle = "";
                $cls = [];
                if (!empty($fdef->showattr[0])) {
                    $tdstyle .=
                        "text-align:" .
                        $fdef->showattr[0] .
                        ";text-wrap:nowrap;";
                }
                if (!empty($fdef->showattr[1])) {
                    if ($at = astedit::evalValue($fdef->showattr[1], $val)) {
                        $tdstyle .= "color:$at;";
                    }
                }
                if (!empty($fdef->showattr[2])) {
                    if ($at = astedit::evalValue($fdef->showattr[2], $val)) {
                        $tdstyle .= "background-color:$at;";
                    }
                }
                if (!empty($fdef->showattr[3])) {
                    if ($at = astedit::evalValue($fdef->showattr[3], $val)) {
                        $cls[] = $at;
                    }
                }
                if (!$tdstyle) {
                    if ($this->fields[$fid]->type === "DECIMAL") {
                        $cls[] = "ast-td-dec";
                    } elseif (
                        in_array($this->fields[$fid]->type, [
                            "INT",
                            "TINYINT",
                            "BIGINT",
                            "INTEGER",
                        ...
                        if (
                            substr($this->fields[$fid]->edittype, 0, 5) ===
                            "CHECK"
                        ) {
                            $cls[] = "ast-td-checkbox";
                        } else {
                ...
                if (count($cls)) {
                    $cellClass = " class='" . implode(" ", $cls) . "'";
                }
                # if ($tdstyle) $tdtags .= "style='$tdstyle'";
            }

            if (substr($fid, 0, 1) == "@" && is_callable(substr($fid, 1))) {
                # user def.field, rendered by func.
                # writeDebugInfo("$fid: calling userfunc ?");
            ...
            if (!empty($fdef->showcond)) {
                #<4>
                $val = $this->GetViewValue($fid, $arRow, $val);
                # $tdtags .= ($this->_drawtdid)? " id=\"{$tdid}\"":'';
                # $ret .= "<td {$tdtags}>$val</td>\n";
            } #<4>
            if(!empty($fdef->showhref)) {
                $href =str_ireplace('{ID}', $pkvalue, $fdef->showhref);
                $val = "<a href=\"$href\">$val</a>";
            }
            if ($cellClass) {
                $val = "<div id=\"$tdid\" $cellClass>$val</div>";
            }
            $arReturn[$fid] = $val;
        }
        $tid = $this->tbrowseid;
        if (count($this->_pkfields) > 0 && empty($this->groupby)) {
            # <3-pkfield>
            $id = $pkvalue;
            ...
            if (count($this->customcol) > 0) {
                foreach ($this->customcol as $no => $custcol) {
                    ...
                    if (substr($turl, 0, 1) == "@") {
                        $turl = astedit::evalValue($turl, $arRow);
                    } else {
                        $turl = str_replace("{ID}", $id, $turl);
                    }
                    $arReturn[
                        "__ccol_{$no}"
                    ] = $custHtm = "<div id=\"$tdid\" nowrap $addon>$turl</div>"; # empty placeholder for "link"
            ...
            if ($this->clonable && !$this->_frozen && count($this->_pkfields)) {
                $arReturn["__clone__"] =
                    "<div class='text-center'><span role='button' onclick=\"asteditJs.astDoAction('$tid','cloneRecord','$id')\" title=\"$ast_tips[tipclone]\">" .
                    '<i class="bi bi-copy font12"></i></span></div>';
            }
            ...
            if (!empty($this->canedit) && count($this->_pkfields) && !$this->_frozen ) {
                ...
                if (is_string($this->canedit)) {
                    if (is_callable($this->canedit)) {
                        $canedit_rec = call_user_func(
                            $this->canedit,
                            $ast_datarow
                        );
                    }
                } elseif (is_array($this->canedit)) {
                    # get biggest eval from all array items
                    $canedit_rec = 0;
                    foreach ($this->canedit as $item) {
                        $caned = is_callable($item)
                            ? call_user_func($item, $ast_datarow)
                            : astedit::evalValue($item);
                        $canedit_rec = max($canedit_rec, $caned);
                    }
                }

                if ($canedit_rec) {
                    # add href/form for editing row
                    $arReturn["__edit__"] =
                        "<div class='text-center'><span role='button' onclick=\"asteditJs.astDoAction('$tid','edit','$id')\" title=\"$ast_tips[tipedit]\">" .
            ...
            if (!empty($this->candelete) && empty($this->_viewmode)) {
                ...
                if ($delthis) {
                    $delCode =
                        "<div class='text-center'><span role='button' onclick=\"asteditJs.astDoAction('$tid','delete','$id')\" title=\"{$ast_tips["tipdelete"]}\">" .
       ...
       */
        ...
        if ($this->_viewmode) {
            $vtxt = isset($ast_tips["link_viewrecord"])
                ? $ast_tips["link_viewrecord"]
                : "View this record";
     ...
     *
     */
    function GetViewValue($fldid, $rowdata = false, $newVal=NULL)
    {
        global $ast_datarow;
        if (!is_array($rowdata)) {
            $rowdata = $ast_datarow;
        }
        if($newVal!==NULL) $rowdata[$fldid] = $newVal;
        if (substr($fldid, 0, 1) == "@") {
            return @call_user_func(substr($fldid, 1), $rowdata);
        }
        ...
        if (true) {
            #<4> !empty($fld->showcond)
            $val = isset($rowdata[$fldid]) ? $rowdata[$fldid] : ""; #$fld->id.' - NO FIELD !');
            $shfunc = $fld->showformula;
            if (!empty($shfunc)) {
                $val = astedit::evalValue($shfunc, $val, $rowdata);
                if (is_array($val)) {
                    $val = "N/A:" . $val;
                }
            } elseif ($efmt[0] === "CHECKBOX") {
                $val = astedit::BrShowBool($val);
            } elseif ($efmt[0] === "SELECT") {
                # auto-decode field value that is selected from known List
                if (!empty($efmt[1])) {
                    $lar = GetArrayFromString($efmt[1]);
                    if (is_array($lar) and count($lar) > 0) {
                        foreach ($lar as $ino => $item) {
                            if (is_array($item)) {
                                if (isset($item[1]) and $item[0] == $val) {
                                    return $item[1];
                                }
                            } elseif ($ino == $val) {
                                return $item;
                ...
                # for empty show formula - browse format depends on field type
                if ($fld->type === "DATE" || $fld->type === "DATETIME") {
                    $val = intval($val)
                        ? to_char($val, $fld->type === "DATETIME")
                        : "";
                }
            ...
            if (!empty($fld->showhref) && !empty($pkvalue)) {
                ...
                $val =
                    '<a href="' .
                    str_replace("{ID}", $pkvalue, $fld->showhref) .
                    '" ' .
                    $haddon .
                    ">$val</a>";
            }
            if (
                $this->_recursive_level > 0 &&
                $fld->id === $this->recursive_show
            ) {
                $val =
                    str_repeat("&nbsp; &nbsp; ", $this->_recursive_level) .
                    $val;
            }
        } #<4>
        return $val;
    ...
    public function setFieldOptions($fieldid, $opts = [])
    {
        if (!isset($this->fields[$fieldid])) {
            return false;
        }
        if (isset($opts["defvalue"])) {
            $this->fields[$fieldid]->defvalue = $opts["defvalue"];
        }
        if (isset($opts["showcond"])) {
            $this->fields[$fieldid]->showcond = $opts["showcond"];
        }
        if (isset($opts["showformula"])) {
            $this->fields[$fieldid]->showformula = $opts["showformula"];
        }
        if (isset($opts["editcond"])) {
            $this->fields[$fieldid]->editcond = $opts["editcond"];
        }
        if (isset($opts["edittype"])) {
            $this->fields[$fieldid]->edittype = $opts["edittype"];
        }
        if (isset($opts["idx_name"])) {
            $this->fields[$fieldid]->idx_name = $opts["idx_name"];
        }
    }
    ...
    function PrepareFilter()
    {
        ...
        if (
            !empty($this->browsefilter_fn) &&
            is_callable($this->browsefilter_fn)
        ) {
        ...
        if (is_array($this->browsefilter) && count($this->browsefilter) > 0) {
            foreach ($this->browsefilter as $flt) {
                if (!empty($flt) && ($realflt = astedit::evalValue($flt))) {
                    $filt[] = "($realflt)";
                }
            }
        } elseif (
            is_string($this->browsefilter) &&
            $this->browsefilter !== ""
        ) {
            $filt[] = "($this->browsefilter)";
        }

        if (!empty($this->userfilter)) {
            $filt[] = "(" . $this->userfilter . ")";
        }
        ...
        if (!empty($this->filterPrefix)) {
            $fltid = $this->filterPrefix . "-" . $fltid;
        }

        $sflds = explode(",", $this->search);
        foreach ($sflds as $fldid) {
            #<3> - for
            $f_arr = explode("^", $fldid);
            ...
            if ($fldid[0] === "@") {
                ...
                $fltexpr = is_callable($sfunc)
                    ? call_user_func($sfunc, "compute")
                    : "";
            } #<4>
            if ($fltexpr) {
                $filt[] = "($fltexpr)";
        ...
        foreach ($this->fields as $fldid => $fld) {
            ...
            if (
                !empty($this->_converters[$fname]) &&
                is_callable($this->_converters[$fname])
            ) {
                $fltexpr = call_user_func($this->_converters[$fname], $fval);
            } else {
                $b_exact = true;
                if (is_array($fval) && count($fval) > 1) {
                    # range search for field
                    $val1 =
                        substr($fld->type, 0, 4) === "DATE"
                            ? dateRepair($fval[0])
                            : $fval[0];
                    $val2 =
                        substr($fld->type, 0, 4) === "DATE"
                ...
                # range case end
                    ...
                    if (substr($fld->type, 0, 4) === "DATE") {
                        # convert from entered dd.mm.yyyy to DB format 'Y-M-D'
                        if (intval($fval) == 0) {
                            $fval = "0";
                        } else {
                            $fval = dateRepair($fval);
                        }
                    }
                    ...
                    if ($edtype == "SELECT") {
                        $b_exact = true;
                    } # varchar but select from list - exact search
                    if ($edtype === "BLIST") {
                        $fltexpr = "FIND_IN_SET('$fval',$fname)";
                    } elseif ($b_exact) {
                        $fltexpr = "$fname='$fval'";
                    } else {
                        # search on substring, " like "val%"
                        if (empty($fulltextindex_support)) {
                            $fltexpr = "$fname LIKE '$fval%'";
                        }
                        # standard like ... options
                        else {
                            $fltexpr = "MATCH($fname) AGAINST('$fval')";
    ...
    function GetDefaultValues(&$param)
    {
        ...
        foreach ($this->fields as $fld) {
            if (!$fld->_autoinc) {
                $param[$fld->id] = str_replace("'", "", $fld->defvalue);
            }
        }
    }
    ...
    public function prepareOrder()
    {
        ...
        if (count($order)) {
            ...
            foreach ($order as $ordItem) {
                ...
                if ($fieldName) {
                    $_SESSION[$baseKey][$fieldName] = [
                        "column" => $fieldName,
                        "dir" => $direction,
                    ];
                    $ordOptions[] =
                        $fieldName . ($direction === "desc" ? " DESC" : "");
                }
            }
        } else {
            # first opening page, seek current session or use default $this->browseorder
            if (isset($_SESSION[$baseKey]) && count($_SESSION[$baseKey])) {
                foreach ($_SESSION[$baseKey] as $ordItem) {
        ...
        if (count($ordOptions)) {
            $this->browseorder = implode(",", $ordOptions);
        }
    ...
    public function drawBrowsePage()
    {
        ...
        $formid = "astgrid_" . $this->tbrowseid;
        $pageCode = "<table id=\"$formid\" class=\"display table table-bordered table-hover table-striped\"><thead><tr>";
        $jsFields = [];
        foreach ($this->viewfields as $fldid => $oneField) {
            $vLabel = !empty($this->fields[$oneField]->shortdesc)
                ? $this->fields[$oneField]->shortdesc
                : $oneField;
        ...
        if (count($this->customcol)) {
            foreach ($this->customcol as $colno => $onecol) {
        ...
        if ($this->clonable && !$this->_frozen) {
            # $pageCode .="<th>Ed</th>"; #
            $pageCode .= "<th><i class='bi bi-copy font12'></i></th>";
        ...
        if ($this->candelete && !$this->_frozen) {
            $pageCode .=
                "<th><i class='bi bi-trash text-danger font12'></i></th>"; # bi bi-x
            $jsFields[] = " { data: '__del__', orderable: false }";
        }

        $pageCode .= "</thead></table>";
        if (!empty($this->browsewidth)) {
            $pageCode = astedit::limitAreaWidth($pageCode, $this->browsewidth);
        }
        $tid = $this->tbrowseid;
        if ($this->caninsert && empty($this->brenderfunc) && empty($this->groupby))
        {
            # "add" is a button on bottom toolbar
            $this->toolBar[] = "<span class=\"ms-auto\"><input type=\"button\" class=\"btn btn-primary\" onclick=\"asteditJs.astDoAction('$tid','add',0)\" value=\"$ast_tips[tipadd]\" /></span>";
        }

        # draw bottom toolbar if any html code exists
        if (count($this->toolBar)) {
            $pageCode .=
                "<div class='area-buttons bounded'>" .
        ...
        if (!empty($this->browseorder) || !empty($_SESSION[$baseKey])) {
            ...
            $initOrder = count($arOrders)
                ? "order: [ " . implode(", ", $arOrders) . "],\n"
                : "";
        }
        $jsFields = implode(",", $jsFields);
        $jsInit = astedit::getCommonJsCode(); # base JS code, common for all datatables ob the page
        # $jsInit .= $this->addJsForTable();
        $brHeight = empty($this->browseHeight)
            ? ""
            : "scrollY: '{$this->browseHeight}',";
        $delTitle = $ast_tips["title_delete"] ?? "Deleting";

        $jsInit .= <<<EOJS
$(document).ready(function() {
  asteditJs.tc['$this->tbrowseid']  = {
    delPrompt: '$ast_tips[deleteprompt]',
    delTitle: '$delTitle',
    postUrl: '$this->baseuri',
    dTable: new DataTable('#{$formid}', {
      ...
      $brHeight scrollCollapse: true,
      layout: {  topStart: null, bottomStart: 'paging',  bottomEnd: 'info' }
    })
  };
});
EOJS;
    ...
    # add specific JS code for defining Datatable
    public function addJsForTable()
    {
        global $ast_tips;
        $delTitle = $ast_tips["title_delete"] ?? "Deleting";
        $brHeight = empty($this->browseHeight)
...
EOJS;
    ...
    public function getPageData()
    {
        ...
        if ($rpp > 0 && empty($this->groupby)) {
            $sqlqry .=
                " LIMIT $this->rpp" . ($start > 0 ? " OFFSET $start" : "");
        }
            ...
            while ($ast_datarow = Astedit::$db->fetch_assoc($r_res)) {
                ...
                if (connection_aborted()) {
                    if (!empty($_SESSION["debug"])) {
                        WriteDebugInfo("USER CONNECTION ABORTED!");
                    }
                    exit();
                }
                if (
                    !empty($this->brenderfunc) &&
                    is_callable($this->brenderfunc)
                ) {
                    $recordItems[] = call_user_func(
                        $this->brenderfunc,
                        $ast_datarow
                    ); # call_user_func_array
                } else {
                    $recordItems[] = $this->browseRow($ast_datarow);
                }
            }
        }
        return $recordItems;
    ...
    function drawInput($fldid, $forsearch = false, $act = "add", $row = 0)
    {
        ...
        if (
            $econd === "C" ||
            (in_array($id, $this->_pkfields) && $act === "edit")
        ) {
            return "";
        }
        ...
        if (in_array($etype, ["SELECT", "BLIST", "BLISTEXT", "BLISTLINK"])) {
        ...
        if (substr($def, 0, 1) === "+") {
            ...
            if ($lnk && ($rw = Astedit::$db->fetch_row($lnk))) {
                $newval = $rw[0];
            }
        }
        # Now - for ALL fields, if it's filtered, use filter value as default in additing, was: elseif($def== '=' || $def == '')
        else {
            # беру по тек.критерию отбора по данному полю
            $curfilt = isset($_SESSION[$fltid][$id])
                ? $_SESSION[$fltid][$id]
                : "*?*";
            if ($curfilt !== "*?*") {
                $disab = "readonly";
                $newval = $curfilt;
            } elseif (empty($newval) && !empty($def)) {
                $newval = $def;
            }
        }
        if (!is_array($row) && isset($newval)) {
            # new record, make date field if 'now' used as 'new value'
            if ($newval == "now") {
                $newval = date("d.m.Y H:i:s");
            } elseif ($newval == "today") {
                $newval = date("d.m.Y");
            }
        }
        ...
        if (empty($econd) && empty($forsearch)) {
            return "";
        } # non-editable fields skipped !
        if (
            ($econd === "E" && $act === "add") ||
        ...
        if ($forsearch === "from") {
            ...
            if ($ar->external) {
                # try to get external field value
        ...
        if ($etype !== "BLISTLINK") {
            if ($intags === "" && count($this->events)) {
                if (
                    isset($this->events[$ftype]) &&
                    is_array($this->events[$ftype])
                ) {
                    $t_ar = $this->events[$ftype];
                }
                if (!empty($t_ar)) {
                    foreach ($t_ar as $ev_type => $ev_func) {
        ...
        if ($forsearch) {
            $editid = "flt_value";
            if ($forsearch === "from" or $forsearch === "to") {
                $editid .= "_" . $forsearch;
            }
        } else {
            $editid =
                ($this->_prefixeditfield ? $this->id . "_" : "") . $ar->id;
        }
        ...
        switch ($etype) {
            ...
            case "":
                ...
                if (stripos($addon, "class=") === false) {
                    if (!empty($as_cssclass["textfield"])) {
                        $fldclasses["txt"] = $as_cssclass["textfield"];
                    }
                }
                if (substr($ftype, 0, 4) === "DATE") {
                    $addon = empty($addon)
                        ? "size='10' maxlength=$elen"
                        : $addon;
                    ...
                    $addon = empty($addon)
                        ? "size='$edlen' maxlength='$elen'"
                        : $addon;
                }

                $txtclass = count($fldclasses)
                    ? ' class="' . implode(" ", $fldclasses) . '"'
                    : "";
                if (!empty($ast_frmfunctions[AST_FORM_ELEMENT][$ftype])) {
                    $ret = call_user_func(
                        $ast_frmfunctions[AST_FORM_ELEMENT][$ftype],
                        $id,
                        $def
                    );
                } else {
                    $ret = "<input type=\"$etype\" id=\"$editid\" name=\"$editid\" $txtclass $addon value='$def' />";
                }
                break;
            ...
            case "DATE":
                if (!empty($ast_frmfunctions[AST_FORM_ELEMENT][$ftype])) {
                    ...
                    $ret = "<input type=\"date\" id=\"$editid\" name=\"$editid\" $txtclass $addon value='$def' />";
                }
                break;
            ...
            case "DATETIME":
                ...
                if (intval($defymd) <= 0) {
                    ...
                    $defymd = to_char($defymd);
                }

                if (!empty($ast_frmfunctions[AST_FORM_ELEMENT]["DATE"])) {
                    $ret = call_user_func(
                        $ast_frmfunctions[AST_FORM_ELEMENT]["DATE"],
                        $id,
                        substr($def, 0, 10)
                    );
                } else {
                    ...
                    $ret .=
                        "&nbsp; $tmtitle: <input type=\"text\" id=\"{$editid}_hh\" name=\"{$editid}_hh\" $txtclass style='width:20px;text-align:center' value='$defhh' maxlength='2'/>" .
                        " : <input type=\"text\" id=\"{$editid}_mm\" name=\"{$editid}_mm\" $txtclass style='width:20px;text-align:center' value='$defmm'  maxlength='2'/>";
                    if (
                        defined(
                            "USE_JQUERY_UI"
                        ) /*&& constant('USE_JQUERY_UI')*/
                    ) {
                        # add UI spinners to hours and mins
                        $hhmmjs = <<<EOJS
$(document).ready(function() {
...
EOJS;
                ...
                switch ($ast_wysiwyg_type) {
                    case "spaw":
                        ...
                        if (class_exists("SpawEditor")) {
                            ...
                            echo "ERROR: spaw editor class not included !";
                        }
                        $ret = ob_get_clean();
                        break;
                    case "tinymce":
            ...
            case "SELECT":
                ...
                $lar = empty($elist) ? [] : GetArrayFromString($elist);
                if (is_array($lar)) {
                    #<3>
                ...
                if (!empty($ar->edittype)) {
                    ...
                    if (!empty($elist)) {
                        ...
                        if ($forsearch) {
                            ...
                            $ret .=
                                DrawSelectOptions($lar, $def, true) .
                                "</select>";
                            $vstring = "";
                        } else {
                            if ($etype === "BLIST") {
     ...
     */
            case "FILE":
                ...
                break;
            case "UDF":
                if (!empty($intags) && is_callable($intags)) {
                    $ret = call_user_func($intags, $ar->id, $act, $row);
                } else {
                    $ret = "undefined UDF: ($intags)";
                }
                break;
            default:
                $ret = $this->debug
                    ? "AStedit: unknown edit type {$ar->id} = $etype"
                    : "";
                break;
        }
        $afterinput = empty($ar->afterinputcode) ? "" : $ar->afterinputcode;
        if ($afterinput) {
            if (substr($afterinput, 0, 1) == "@") {
                $afterinput = eval($afterinput);
            } else {
                $afterinput = str_replace(
                    ["{imgpath}", "{ID}"],
                    [IMGPATH, $this->FullPkFieldAsString($row)],
                    $afterinput
                );
            }
            $ret .= "&nbsp;" . $afterinput;
        ...
        if ($ar->subtype === "LOGIN" && !$forsearch) {
            $ret .= " &nbsp;<span id='chk_{$this->id}_{$ar->id}'>&nbsp; </span>";
        } # here will be drawn check result
        if ($forsearch) {
            $ret =
                ($forsearch === "from" || $forsearch === "to"
                    ? ""
                    : $ar->shortdesc . " ") . $ret;
        }
        return $ret;
    ...
    function DrawFormElement($fldid, $act, $row = 0, $fullform = 1)
    {
        ...
        if (isset($this->fixFields[$fldid])) {
            return "<input type='hidden' name='$fldid' value='" .
                $this->fixFields[$fldid] .
                "' />";
        } else {
        ...
        if (
            $econd === "C" ||
            (in_array($fldid, $this->_pkfields) && $act === "edit")
        ) {
        ...
        if (!empty($ar->editRowClass)) {
            $relem .= " class=\"$ar->editRowClass\"";
        }

        # $addon .= (empty($addon)?'':' ').$inputattrib; # READONLY or whatelse if needed
        $prompt = '<td class="text-end" >' . $ar->desc . "</td>"; # <td>title</td> if needed
        if ($fullform) {
            # full screen form, so add all tags
            $ptompt = $ar->desc;
    ...
    {
        $vparam = str_replace(["&amp;", "&"], ["&amp;", "&amp;"], $vparam); # to avoid showing '&lt;' as '<' etc.
    }
    ...
    function DrawBinaryList($flid, $lar, $def = "")
    {
        ...
        if (is_array($lar) && count($lar) > 0) {
            #<3>
            $spinit = is_array($def) ? $def : explode(",", $def);
            foreach ($lar as $kkk => $item) {
                #<4>
                if (is_array($item) && $item[0] === "<") {
                    # optgoup - title before sub-list
    ...
    function DrawBinaryListExt($flid, $lar, $def = "")
    {
        ...
        if (is_array($lar) && count($lar) > 0) {
            ...
            foreach ($lar as $kkk => $item) {
                ...
                $ret .=
                    "<tr><td><label><input type='checkbox' name='{$flid}[]' id='_tmp_{$flid}_$pid' value='$pid' $chk /> $pname</label></td>" .
    ...
    function DrawBinaryListLink($flid,$lar,$def = "",$func = "",$lnkTitle = "") {
        ...
        $blext_w = 30; #default width for additiona numeric field
        if (empty($func)) {
            $func = $editElems[2] ?? "astedit.openSubForm('{id}')";
        }
        if (empty($lnkTitle)) {
            $lnkTitle = $editElems[3] ?? "Add Params";
        }
        ...
        if (is_array($lar) && count($lar) > 0) {
            ...
            foreach ($lar as $kkk => $item) {
                ...
                $btnHtml = $hide
                    ? ""
                    : "<td><input type='button' class='btn btn-primary' onclick=\"$oneFunc\" value='$lnkTitle'></td>";
                $ret .=
                    "<tr><td><label><input type='checkbox' name='{$flid}[]' value='$pid' $chk /> $pname</label></td>" .
                    $btnHtml .
                    "</td></tr>\n";
            } #<4>
        } #<3>
    ...
    public function DrawEditForm(
        ...
        if (!empty($GLOBALS["ast_act"])) {
            $ast_act = $GLOBALS["ast_act"];
        }
        if (!empty($GLOBALS["ast_edit_id"])) {
            $id = $GLOBALS["ast_edit_id"];
        }
        ...
        $submittxt =
            $ast_act == "edit" ? $ast_tips["buttsave"] : $ast_tips["buttadd"];
        $canceltxt = isset($ast_tips["buttcancel"])
            ? $ast_tips["buttcancel"]
            : "Cancel";

        $btnclass = empty($as_cssclass["button"])
            ? ""
            : "class='{$as_cssclass["button"]}'";
        ...
        if ($ast_act == "edit" && !empty($id)) {
            ...
            $ast_datarow = Astedit::$db->select($this->id, [
                "where" => $this->pkEqExpression($id),
                "singlerow" => 1,
            ]);
            # writeDebugInfo("data row: ", $ast_datarow );
        }
        $js = Astedit::getCommonJsCode();
        $js .= $this->addJsForTable();
        if ($this->helppage) {
            $js .= <<<EOJS
...
EOJS;
        ...
        if (!$returnBody) {
            ...
            if ($backlink === true && empty($this->windowededit)) {
                $strRet .= "<a href=\"{$this->baseuri}\" >{$ast_tips["tobrowse"]}</a>\n";
            }
        }
        ...
        if (count($this->_wysiwyg) > 0 && $ast_wysiwyg_type == "openwysiwyg") {
            ...
            foreach ($this->_wysiwyg as $wysid) {
                $strRet .= "WYSIWYG.attach('$wysid'); alert('$wysid');\n";
            }
            $strRet .= "</script>";
        }
        # simple built-in WYSIWYG editor
        if (count($this->_wysiwyg) && $ast_wysiwyg_type == "")  {
            $strRet .= <<< EOJS
...
EOJS;
            ...
            foreach ($this->_wysiwyg as $wfld) {
                $strRet .= "    SaveWysiwygField('$wfld');\n";
            }
            $strRet .= "return true; }\n</script>";
        ...
        $frmtags = "";
        if ($this->_multipart) {
            $frmtags .= ' enctype="multipart/form-data"';
        }
        # if ($onsubmit) $frmtags .= " onsubmit='$onsubmit'";
        if (!$returnBody) {
            $strRet .= "<form name=\"astedit_{$this->id}\" id=\"astedit_{$this->id}\" $frmtags >
        <div class='card'><table class='table align-middle'>";
        } else {
            $strRet .= "<form name=\"astedit_{$this->id}\" id=\"astedit_{$this->id}\"  $frmtags ><table class='table' >";
        }

        $strRet .= "<input type=\"hidden\" id=\"ast_act\" name=\"ast_act\" value=\"do{$ast_act}\" />";

        if (!empty($id)) {
            $strRet .= "<input type=\"hidden\" id=\"_astkeyvalue_\" name=\"_astkeyvalue_\" value=\"$id\" />";
        }
        if (!empty($this->edit_template)) {
            ...
            foreach ($this->fields as $fldid => $fld) {
                ...
                $fsubst["%" . $fldid . "_input%"] = $this->drawInput(
                    $this->fields[$fldid],
                    false,
                    $ast_act,
                    $ast_datarow
            ...
            $strRet .= $frmBody;
            if (count($this->fixFields)) {
                foreach ($this->fixFields as $fixKey => $fixVal) {
                    $strRet .= "<input type='hidden' name='$fixKey' value='$fixVal' />";
                };
            }
        } else {
            foreach ($this->fields as $fldid => $fld) {
                $strRet .= $this->DrawFormElement(
                    $fldid,
                    $ast_act,
                    $ast_datarow
                );
        ...
        if (!empty($this->editsubform)) {
            if (is_callable($this->editsubform)) {
                $code = call_user_func_array($this->editsubform, [
                    $ast_act,
                    $id,
                ]);
                if (!empty($code) && is_string($code)) {
                    $strRet .= "<tr><td colspan=\"2\">$code</td></tr>";
                }
            }
        }
        $canc = $this->windowededit
            ? "<input type=\"button\" onClick=\"window.close()\" $btnclass value=\"$canceltxt\" />"
            : "";
        $helpBtn = "";
        if ($this->helppage && !$returnBody) {
            $helpBtn = "<a class=\"helplink\" href='javascript:asteditJs.showHelp(\"$this->tbrowseid\")' title=\"$ast_tips[tiphelpedit]\">?</a>";
        }

        if (
            !empty($this->aftereditsubform) &&
            is_callable($this->aftereditsubform)
        ) {
            $code = call_user_func_array($this->aftereditsubform, [
                $ast_act,
                $id,
            ]);
            if (!empty($code) && is_string(code)) {
        ...
        if ($returnBody) {
            ...
            $strRet .= "<div class='card-footer justify-content-between'><span></span> <span><input type='button' id='ast_submit_edit' {$btnclass} value='$submittxt' onclick=\"asteditJs.astDoAction('$tid','update','$this->id')\"/> $canc</span> <span>$helpBtn</span></div>\n";
            $strRet .= "</form>\n";
            $strRet .= "</div> <!-- editing_record -->\n";
        }
        if ($this->editformwidth) {
            $strRet = astedit::limitAreaWidth($strRet, $this->editformwidth);
        }
        if ($returnBody) {
            return $strRet;
        }
    ...
    */
    function UpdateDataIntoTable($act, $pkeyval = 0, $newdata = false)
    {
        ...
        $keyvalues = "";
        if (!empty($params["_astkeyvalue_"])) {
            $keyvalues = $params["_astkeyvalue_"];
        } elseif (!empty($pkeyval)) {
            $keyvalues = $pkeyval;
        } else {
            foreach ($this->_pkfields as $pkf) {
                $keyvalues .=
                    ($keyvalues == "" ? "" : AST_PKDELIMITER) .
                    (isset($params[$pkf]) ? $params[$pkf] : "");
            };
        }
        ...
        if (!empty($this->afteredit)) {
            if (is_callable($this->afteredit)) {
                # check/modify edited fields or do something else before saving
        ...
        $vlst = []; # assoc.array: $vlst['pole1'] => 'pole 1 value' ...
        $uqry = "none";
        ...
        if (count($this->_savefile_pm) > 0) {
            foreach ($this->_savefile_pm as $fldname => $svdata) {
                if (isset($_FILES[$fldname])) {
                    ...
                    if (
                        !empty($svdata[1]) &&
                        !empty($_FILES[$fldname]["type"])
                    ) {
                        $params[$svdata[1]] = $_FILES[$fldname]["type"];
                    } # Save original file type: "image/png"
                }
            }
        }
        foreach ($this->fields as $fid => $fld) {
            ...
            if ($act === "doedit" && in_array($fid, $this->_pkfields)) {
                continue;
            } # dont change primary keys
            $ftype = $fld->type;
            ...
            if (
                strtoupper($fetype[0]) === "CHECKBOX" &&
                !isset($params[$fld->id])
            ) {
                $params[$fld->id] = "0";
            }

            $econd = $this->_updt_all
                ? true
                : astedit::evalValue($fld->editcond);
            $editfld = isset($params[$this->id . "_" . $fid])
                ? $this->id . "_" . $fid
                : $fid;
            if ($econd === "A" and $act !== "doadd") {
                continue;
            }
            ...
            if ($fld->subtype === "LOGIN") {
                if ($standalone) {
                    if ($val === "") {
                        return "Пустое значение для логин-полей недопустимо, операция отклонена !";
                    }
                    $result = preg_match("/^[@.a-z0-9_]*$/i", $val);
                    if (!$result) {
                        return "Значение логина содержит недопустимые символы, операция отклонена !";
                    }
                    if (class_exists("CAuthCorp")) {
                        $auth = CAuthCorp::getInstance();
                        if ($auth->IsBuiltinAccount(strtolower($val))) {
                            return "Выбранный логин запрещен !";
                        }
                    }
                }
                $where =
                    "($fid='$val')" .
                    ($act === "doedit"
                        ? " AND NOT (" . $this->pkEqExpression($keyvalues) . ")"
                        : "");
                $cnt = Astedit::$db->getQueryResult(
                    $this->id,
                    "COUNT(1)",
                    $where
                );
                if ($cnt > 0) {
                    return "Выбранный логин уже был занят, выберите другой. Операция ввода отклонена ! !";
                }
            }
            ...
            if ($econd === "C" && $act === "doadd") {
                ...
                if ($newval === "now" || $newval === "{now}") {
                    $val = date("Y-d-m H:i:s");
                } elseif ($newval === "today" || $newval === "{today}") {
                    $val = date("Y-d-m");
                } else {
                    $val = astedit::evalValue($newval);
                }
                if (!empty($val)) {
                    $vlst[$fld->id] = $val;
                }
                ...
                !empty($econd) &&
                (isset($params[$fld->id]) ||
                    isset($params[$this->id . "_" . $fld->id]) ||
                    in_array($fetype[0], ["BLIST", "BLISTEXT", "BLISTLINK"]) ||
                    $act === "doedit")
            ) {
                #<3>
                # $this->_prefixeditfield - returned fields may contain tablename_ prefix
                if ($val == "" && IsNumberType($ftype, 1)) {
                    $val = "0";
                }
                if (in_array($fetype[0], ["BLIST", "BLISTEXT", "BLISTLINK"])) {
                    # <4> merge _tmp_$flid_$val into "n1,n2,..." text field
                    # TODO: refactor to _tmp_field_blist[] (array input) !
                    $val = (isset($params[$fld->id]) && is_array($params[$fld->id]))
                        ? implode(",", $params[$fld->id])
                        : "";
                    # exit("final blist/ext value:" . $val);
                } #<4>
                if ($ftype === "DATE") {
                    if (in_array($val, ["now", "{now}", "today", "{today}"])) {
                        $val = date("Y-m-d");
                    } else {
                        $val = to_date($val, 2);
                    }
                    ...
                    if (empty($val)) {
                        ...
                        if (isset($def[1]) &&
                            in_array($def[1], ["now","{now}","today","{today}",])) {
                            $val = date("Y-m-d");
                        }
                    } else {
                        $val = to_date($val);
                        if (isset($params[$fld->id . "_hh"])) {
                            $val .=
                                " " .
                                str_pad(intval($params[$fld->id . "_hh"]),2,"0",STR_PAD_LEFT) .
                                ":" .
                                (isset($params[$fld->id . "_mm"])
                                    ? str_pad(intval($params[$fld->id . "_mm"]),2,"0",STR_PAD_LEFT)
                                    : "00");
                        } else {
                            $val = to_date($val, 2);
                        }
                    ...
                    if (empty($this->_enablesinglequote)) {
                        $val = str_replace("'", '"', $val);
                    }
      ...
      */
        $result = -1;
        if (count($vlst) > 0) {
            # <2> values OK, perform update/add action
            if ($act == "doedit") {
                $result = Astedit::$db->update(
                    $this->id,
                    $vlst,
                    $this->pkEqExpression($keyvalues)
                );
            } else {
        ...
        if ($result) {
            ...
            if ($act === "doadd") {
                # конструирую полное значение ключа новой записи
                ...
                $keyvalues = "";
                foreach ($this->_pkfields as $tf) {
                    $keyvalues .=
                        ($keyvalues ? AST_PKDELIMITER : "") . $params[$tf];
                }
            }
            $res_text = Astedit::$VERBOSE
                ? $ast_tips["updated"]
                : (isset($GLOBALS["ast_text_added"])
                    ? $GLOBALS["ast_text_added"]
                    : "Data saved");
            if (is_object($this->_picobj)) {
                #delete old/insert new photos
            ...
            if (count($this->_savefile_pm) > 0) {
                foreach ($this->_savefile_pm as $fldname => $svdata) {
                    ...
                    if (
                        isset($_FILES[$fldname]) &&
                        !empty($_FILES[$fldname]["tmp_name"])
                    ) {
                        # $rec_id, $filename='',$tmp_filename
                        if (!empty($svdata[2]) and is_callable($svdata[2])) {
                            call_user_func($svdata[2],$new_id,$_FILES[$fldname]);
                        }
                        # Save original file name "myfile.png"
                        else {
                            $this->saveUploaded($new_id,$fldname,$_FILES[$fldname]);
                        }
                    }
                }
            }
            ...
            $res_text =
                "Error on executing operator:<br>$uqry<br>" .
                Astedit::$db->sql_error();
            $this->updateResult = "error";
            $this->errUpdate = true;
        }
        return $res_text;
    ...
    public function saveUploaded($id, $fileprefix, $finfo)
    {
        ...
        if (is_array($finfo) && !empty($finfo["tmp_name"])) {
            ...
            if (!$result) {
                ...
                AppAlerts::raiseAlert(
                    "DOC FILES WRITE",
                    "Не удалось записать файл в папку документов - " .
                        $fullfolder
                );
                echo "<div class=\"alarm\">Не удалось записать файл в папку документов - $fullfolder</div>";
            } else {
                AppAlerts::resetAlert("DOC FILES WRITE");
            }
    ...
    public function SetFieldParameters(
        $fld_id,
        $defvalue = "",
        $showcond = "",
        $editcond = ""
     ...
     */
    public function FreezeTable($parm = true)
    {
        $this->_frozen = $parm;
    }
    ...
    public function MainProcess($dobrowse = 1, $canedit = null, $candelete = null, $caninsert = null)
    {
        ...
        global $ast_act, $id, $ast_tips, $imgPath, $cursortfld, $cursortord;
        $isAjax = isAjaxCall();
        if ($canedit !== null) {
        ...
        if($this->_viewmode) $this->candelete = $this->caninsert = 0; // $this->canedit
    ...
    */
        ...
        if (empty($this->id)) {
            echo "No meta-data loaded";
            return;
        }
        if (empty($ast_act) && !empty($ast_parm["ast_act"])) {
            $ast_act = $ast_parm["ast_act"];
        }
        if ($ast_act === "clone") {
            $id = isset($ast_parm["sourceid"]) ? $ast_parm["sourceid"] : 0;
            if ($id > 0 && $this->clonable && $caninsert) {
                $this->CloneRecord($id);
            }
        }
        ...
        if (empty($ast_act)) {
            $ast_act = empty($ast_parm["ast_act"]) ? "" : $ast_parm["ast_act"];
        }

        if (empty($id)) {
            $id = empty($ast_parm["id"]) ? "" : $ast_parm["id"];
        }
        ...
        if (
            ($ast_act === "doedit" || $ast_act === "delete") &&
            empty($id) &&
            !empty($ast_parm["_astkeyvalue_"])
        ) {
            $id = $ast_parm["_astkeyvalue_"];
        } # <TODO>!!!
        ...
        switch ($ast_act) {
            ...
            case "delete":
                ...
                if (!isset($ast_tips["deleteprompt"])) {
                    $ast_tips["deleteprompt"] =
                        "Deleting record. Are You sure ?";
                }
                if ($this->_frozen) {
                    ...
                    if (
                        !empty($this->recdeletefunc) &&
                        is_callable($this->recdeletefunc)
                    ) {
            ...
            case "checkunique": # 'login' field changed: check if value is unique
                ...
                if (is_callable("WebApp::getLocalized")) {
                    foreach ($loc_str as $msg_id => &$val) {
                        if ($strk = WebApp::getLocalized($msg_id)) {
                            $val = $strk;
                        }
                    }
                }

                $field = isset($ast_parm["fieldid"])
                    ? $ast_parm["fieldid"]
                    : "";
                ...
                $where =
                    "$field='$fval'" .
                    ($recid > 0 ? " AND $this->_pkfield <> $recid" : "");
                # WriteDebugInfo('обработка check_loginunique, where=', $field);

                $result = preg_match("/^[a-z0-9_@.\-]*$/i", $fval);
                if (!$result) {
                    $ret = $loc_str["err_login_wrong_chars"];
                }
                # "Значение логина содержит недопустимые символы !";
                else {
                    $ret = "";
                    if (
                        !empty(Astedit::$checkLoginFunc) &&
                        is_callable(Astedit::$checkLoginFunc)
                    ) {
                        # check login uniquness by user function
                        $exists = call_user_func(
                            Astedit::$checkLoginFunc,
                            $fval
                        );
                        if ($exists) {
                            $ret =
                                $loc_str["err_login_used"] .
                                (is_string($exists) ? " $exists" : "");
                        }
                    } else {
                        if (class_exists("CAuthCorp")) {
                            $auth = CAuthCorp::getInstance();
                            if ($auth->IsBuiltinAccount(strtolower($fval))) {
                                $ret = $loc_str["err_login_used"];
                            } #'Выбранный логин запрещен !';
                        }
                        if ($ret === "") {
                            $cnt = Astedit::$db->getQueryResult(
                                $this->id,
        ...
        if ($showbrowse) {
            #<2> draw base HTML code for grid, filters...
     ...
     * send one page table data block as JSON in datatables.js formst
     */
    public function handleAjaxRequest()
    {
        ...
        if (!empty($this->_rawFilter)) {
            $selParams["where"] = $this->_rawFilter;
        }
        ...
        switch ($ast_act) {
            case "setfilter": # user sets some search criteria - so set a filter
                ...
                if (!empty($ast_parm["search_function"])) {
                    ...
                    if (is_callable($searchfnc)) {
                        call_user_func($searchfnc, "setfilter");
                    }
                } else {
                    if (empty($fname) || !isset($this->fields[$fname])) {
                        exit("ERROR: Seek field name wrong or not passed");
                    }
                    $val = isset($ast_parm["flt_value"])
                        ? $ast_parm["flt_value"]
                        : "";
                    $valfrom = isset($ast_parm["flt_value_from"])
                        ? $ast_parm["flt_value_from"]
                        : null;
                    $valto = isset($ast_parm["flt_value_to"])
                        ? $ast_parm["flt_value_to"]
                        : null;
                    if (!empty($ast_parm["clearfilter"])) {
                        unset($_SESSION[$tid][$fname]);
                    } else {
                        if (
                            $this->fields[$fname]->edittype == "CHECKBOX" or
                            $this->fields[$fname]->type == "BOOL"
                        ) {
                            $val = empty($val) ? 0 : 1;
                        }
                        if ($valfrom !== null or $valto !== null) {
                            $_SESSION[$tid][$fname] = [$valfrom, $valto];
                        } else {
                            $_SESSION[$tid][$fname] = $val;
                        } # single value filter
                    }
                }
                exit("1");
            ...
            case "resetfilter":
                if (!empty($ast_parm["search_function"])) {
                    $searchfnc = $ast_parm["search_function"];
                    if (is_callable($searchfnc)) {
                        call_user_func($searchfnc, "reset");
                    }
                } else {
                    $tid = "flt_" . $this->tbrowseid; # table name, must exist in passed attributes
                    if (!empty($this->filterPrefix)) {
            ...
            case "doedit":
            case "doadd":
                $resultText = $this->UpdateDataIntoTable(
                    $ast_act,
                    0,
                    $ast_parm
                );
            ...
            case "delete":
                $id = $ast_parm["id"] ?? 0;
                if (empty($id)) {
                    exit("1" . AjaxResponse::showError("No ID passed"));
                }
            ...
            case "showhelp":
                ...
                if ($pageFile == 1) {
                    $pageFile = $this->tbrowseid . ".help";
                }
                $helpFile = dirname($this->tplfile) . "/$pageFile.htm";
                if (is_file($helpFile)) {
                    exit(file_get_contents($helpFile));
                } else {
                    exit($ast_tips["help_page_notfound"]);
                }
                break;
            case "askCloneRecord":
            ...
            default:
                exit(
                    "1" .
                        ajaxResponse::showError(
                            "Unsupported ast_act: [$ast_act]"
                        )
                );
        }
    }
    private function askCloneRecord($ast_parm)
    {
        ...
        if (!$recid) {
            exit("Wrong call, no record ID");
        }
        $fldid = $this->clonable_field;
        # writeDebugInfo("$fldid=[$fldid], def: ", ($this->fields[$fldid] ?? 'undef'));
        $fldPrompt = isset($this->fields[$fldid])
            ? $this->fields[$fldid]->desc
            : $fldid;
        ...
        $childs = $this->confirm_clone_child
            ...
            : "";
        $parval = $this->confirm_clone_child
            ? '$(\'input#clonechild\').is(\':checked\') ? 1:0'
            : "1";
        $htmlCode =
            "<form id=\"fm_astclone\"><input type='hidden' name='ast_act' value='performCloneRecord'/>" .
            "<input type='hidden' name='_srcid_' value='$recid'/>" .
            $fldPrompt .
            ":<br><input type=\"text\" name='cloned_newvals' id=\"cloned_newvals\" class=\"form-control w300\">$childs<br><br>$ast_tips[clone_record] ?</form>";
    ...
    private function checkUniqueValue($ast_parm)
    {
        ...
        $response =
            "1" . AjaxResponse::setHtml("chk_{$this->id}_{$field}", $ret);
        $response .= AjaxResponse::enable(
            "ast_submit_edit",
            $goodValue ? "1" : "0"
        );
        exit($response);
    ...
    function SetEditJsCode($strcode)
    {
        $this->_edit_jscode = $strcode;
    }
    ...
    function updateFilters()
    {
        global $ast_parm;
        if (
            isset($ast_parm["ast_act"]) &&
            "setfilter" == $ast_parm["ast_act"]
        ) {
            $fltname = $ast_parm["flt_name"];
            if (count($this->reset_chain)) {
                foreach ($this->reset_chain as $reseter) {
                    $sbros = false;
    ...
    function SetFieldAttribs($fldname, $prm1, $edit = "", $newval = "")
    {
        ...
        if (strtolower(get_class($prm1)) == "cfielddefinition") {
            $this->fields[$fldno] = $prm1;
        } else {
            $this->fields[$fldno]->showcond = $prm1;
            if ($edit !== "") {
                $this->fields[$fldname]->editcond = $edit;
            }
            if ($newval !== "") {
                $this->fields[$fldname]->defvalue = $newval;
            }
        }
    }
    /**
    ...
    function DeleteRecord($arData)
    {
        ...
        if (
            Astedit::$RO_mode ||
            !empty($_SESSION["astedit_readonly"]) ||
            $this->_frozen
        ) {
            $this->errUpdate = true;
            return $ast_tips["err_readonly_mode"];
        ...
        if (count($this->childtables) > 0) {
            #<4>

            $del_sql = []; # fill with "DELETE FROM <child_table> ..." operators for each child table
            $thisrec = Astedit::$db->select($this->id, [
                "where" => $this->pkEqExpression($id),
                "singlerow" => 1,
            ]);
            if (is_array($thisrec)) {
                #<5>
                foreach ($this->childtables as $onet) {
                    #<6>
                    $whcond = [];
                    ...
                    if (!empty($onet["condition"])) {
                        $whcond[] = $onet["condition"];
                    }
                    ...
                    if (!empty($onet["protect"])) {
                        ...
                        if ($cnt > 0) {
                            $del_warning[] = intval($onet["protect"])
                                ? self::localize("err_record_cant_be_deleted") .
                                    $onet["table"]
                                : $onet["protect"];
                        }
                    } else {
                        $del_sql[] = ["table" => $onet["table"],"cond" => $whcond];
                    }
                }
        ...
        if (is_object($this->_picobj)) {
            $this->_picobj->DropFiles($id);
        } # delete image files and records from
        ...
        if (count($del_warning) == 0 && count($this->_savefile_pm) > 0) {
            foreach ($this->_savefile_pm as $fldname => $svdata) {
                ...
                if (!empty($svdata[2]) and is_callable($svdata[2])) {
                    call_user_func($svdata[2], $id, false);
                }
                # Save original file name "myfile.png"
                else {
                    $this->dropUploaded($id, $fldname);
                }
            }
        }
        ...
        if (
            !empty($whereExp) ||
            (is_array($whereExp) && count($whereExp) > 0)
        ) {
            Astedit::$db->delete($this->id, $whereExp);
            if (Astedit::$db->sql_errno()) {
                $ret = $ast_tips["rec_delerror"];
            } else {
                $ret = "1";
            }
        } else {
            $ret = "Error - no key field in table";
        }
    ...
    {
        $this->viewfields = is_string($ar_fields)
            ? preg_split("/[\s,;|]+/", $ar_fields)
            : $ar_fields;
        foreach ($this->viewfields as $fid) {
            if (isset($this->fields[$fid])) {
                $this->fields[$fid]->showcond = in_array(
                    $this->fields[$fid]->showcond,
                    ["1", "S"]
                )
                    ? $this->fields[$fid]->showcond
                    : 1;
    ...
    {
        global $ast_tips;
        if (Astedit::$RO_mode || $this->_frozen) {
            return $ast_tips["err_readonly_mode"] ?? "Frozen state!";
        }
        ...
        if (count($this->_pkfields) < 1) {
            exit(
                $ast_tips["err_clone_nopkey"] ??
                    "No primary keys in table, cloning impossible"
            );
        }
        if (empty($record_id)) {
        ...
        if (Astedit::$db->affected_rows() < 1) {
            return 0;
        }

        foreach ($this->_pkfields as $pkf) {
            if ($this->fields[$pkf]->_autoinc) {
                unset($dta[$pkf]);
            }
        }
        if ($nevals == "") {
            ...
            $newArr = preg_split("/[,;\|]/", $nevals, -1, PREG_SPLIT_NO_EMPTY);
        }
        ...
        foreach ($newArr as $itemNo => $newval) {
            if (!empty($this->clonable_field)) {
                if (!empty($newval)) {
                    ...
                    $dta[$this->clonable_field] .=
                        "_clone" . ($itemNo > 0 ? $itemNo : "");
                }
            }

            Astedit::$db->insert($this->id, $dta);
            if (Astedit::$db->affected_rows() == 1) {
                $ret = Astedit::$db->insert_id();
            }

            # if there are child tables, clone related records (if user confirmed)
            if ($ret && $cloneChild && count($this->clone_subs) > 0) {
                #<2>
                ...
                foreach ($this->clone_subs as $tblname => $fkfield) {
                    #<3>
    ...
    {
        if (!empty($funcname)) {
            $this->_converters[$fieldname] = $funcname;
        } else {
            unset($this->_converters[$fieldname]);
        }
    }
    /**
...
{
    $pth = IMGPATH;
    if ($obj->wwtoolbar_ready) {
        return "";
    }
    $obj->wwtoolbar_ready = 1;
    return "<table id='WysiwigToolbar' border=0 cellspacing=0 cellpadding=0><tr>
